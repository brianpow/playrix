...
void crc_trans(WORD index,WORD v)
{
	...
	for(i=1;i<=11;i++)
	{
		res = bit_mult(low,high,0x6A,0);
		low = res; high = res>>16;
		res = fourier_maybe(low,high,0x64,0);
		low = res; high = res>>16;
		buffer[index*12+i] = (low+4)>>3;
	}
}
/*----------------------------------------------------------*/
...
void prepare_a0b0(word index,word v)  /* important !*/
{
	...
	res = fourier_maybe(low,high,0x2000,0);
	low = res&0xFFFF;
	if(low < 0)
	{
...
/*--------------------------------------------------------------*/
DWORD fourier_maybe(WORD parm1,WORD parm2,WORD parm3,WORD parm4)
{
	WORD ax = 0,bx = 0,cx = 0,dx = 0,i = 0;register BYTE CF = 0;
	DWORD result = 0,temp = 0;
	if((short)parm2 < 0)        /* if(parm2 < 0) */
	{
		i++;
		parm2 = (~parm2)&0xFFFF;
		parm1 = (~parm1+1)&0xFFFF;
	}
	if((short)parm4 < 0)		/* if(parm4 < 0) */
	{
		i++;
		parm4 = ~parm4;
		parm3 = ~parm3+1;
	...
	{
		dx = parm2/parm3,
		result = ((DWORD)(parm2%parm3)<<16)+parm1,
		ax = result/parm3,
		result = ((DWORD)dx<<16)+ax;
	}
	else
	{
		bx = parm4,cx = parm3,dx = parm2,ax = parm1;
		do
		{
			CF = (bx&0x0001)==1?1:0,
			bx>>=1,
			cx = CF==1?(cx>>1|0x8000):cx>>1,
			CF = (dx&0x0001)==1?1:0,
			dx>>=1,
			ax = CF==1?(ax>>1|0x8000):ax>>1;
		}
		while(bx != 0);
		result = ((DWORD)dx<<16)+ax,
		bx = ax = (result/cx)&0xFFFF,
		result = ax*parm4,
		cx = result&0xFFFF,
		result = parm3*bx,
		dx = result>>16,
		ax = result&0xFFFF;
		temp = (DWORD)dx+(DWORD)cx;
		dx = dx+cx;
		if((temp > (DWORD)parm2) || (temp>=(DWORD)parm2&&ax>parm1)) bx--;
		dx = 0,
		ax = bx;
		result = ((DWORD)dx<<16)+ax;
	}
...
/*-------------------------------------------------------------------------*/
DWORD bit_mult(WORD parm1,WORD parm2,WORD parm3,WORD parm4)
{
	DWORD result = 0;
	WORD temp = 0;
	if(parm2==0 && parm4==0)
	{
		result = (DWORD)parm1*(DWORD)parm3;
		return result;
	}
	else
	{
		...
		result = parm1*parm4,
		temp += result,
		result = (DWORD)parm3*(DWORD)parm1,
		result += ((DWORD)temp<<16);
		return result;
	}
}
/*----------------------------------------------------------------------*/
DWORD strm_and_fr(WORD parm)
{
	...
	ax = parm;
	ax <<= 1; ax += parm; ax <<= 1;
	dx = (ax&0x8000)==0x8000?0xFFFF:0;
	dx += ax>0xD8EF?1:0;
	ax += 0x2710;
	result = bit_mult(ax,dx,0xCB78,0);
	ax = result; dx = result>>16;
	result = bit_mult(0x2710,0,0x19,0);
	result = fourier_maybe(ax,dx,result,result>>16);
	ax = result; dx = result>>16;
	result = bit_mult(ax,dx,0x4000,2);
	result = fourier_maybe(result,result>>16,0xB503,1);
	return result;
}